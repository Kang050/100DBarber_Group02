...
if (isset($_POST['btnSubmit'])):
    ...
    if ($rsappointments):
        ...
        header("location:appolist.php?msgOK=Đặt lịch thành công!");
    else:
        header("location:appolist.php?msgErr=Đặt lịch không thành công!");
    endif;
endif;
?>
<!DOCTYPE html>
                            ...
                            <div class="row justify-content-center">
                                <div class="col-md-6">
                                    <div class="control-group">
                                        ...
                                        <select id="shift" name="shift" onchange="check()" class="form-control" style="border: 2px solid #1d2434;margin-bottom:20px" required>
                                            <option value="no" >Chọn giờ cắt</option>
                                            <?php
                                            while ($fieldhour = mysqli_fetch_array($rshour)) :
                                                ...
                                                if (strtotime($shiftdate) < strtotime($datetime)):
                                                    ?>
                                                    <option value="<?= $fieldhour[1] ?>" disabled><?= $fieldhour[1] . ' - Quá thời gian' ?></option>
                                                    <?php
                                                else:
                                                    if (!($fieldhour[3] == 1)):
                                                        ?>
                                                        <option value="<?= $fieldhour[1] ?>"><?= $fieldhour[1] ?></option>
                                                        <?php
                                                    else:
                                                        ?>
                                                        <option value="<?= $fieldhour[1] ?>" disabled><?= $fieldhour[1] . ' - Đã kín lịch' ?></option>
                                                    <?php
                                                    endif;
                                                endif;
                                            endwhile;
